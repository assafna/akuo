...
- [ ] **Release bundle built once.** Run `swift test` and `AKUO_CODE_SIGN_IDENTITY='Apple Development: Your Name (TEAMID)' Scripts/build-app.sh release`; record the passing test count, absolute bundle path, executable SHA-256, signing identity, and designated requirement. Do not rebuild or modify this candidate during the remaining metadata and installation steps. **Result:**
  **Evidence:**
