...
`Sources/AkuoCore/AkuoCoreVersion.swift` is the single committed authority for the candidate marketing version and build number. The current `Configuration/Akuo-Info.plist` deliberately contains neither bundle-version key. Packaging rejects every tracked symlink or gitlink before extraction, archives the captured `HEAD` object into a temporary source tree, injects that SHA into a runtime constant in the snapshot, and compiles only the snapshot, so ignored files, external symlink targets, and transient caller-worktree edits cannot enter the executable. It injects the version, build, and same source SHA into the copied candidate plist. Verification compares the authoritative declaration with the packaged plist and compares both the plist and executable runtime source revision with `HEAD`. It compares local `v*` tag objects with the live origin result, then inspects each tagged commit, including a matching legacy tagged plist only for an exact live-origin `v<marketing-version>` tag, so an Unreleased candidate cannot reuse a released version/build pair.

A rebuild of the exact same clean Git commit with the same identity is the same candidate. For an Unreleased build, `Scripts/build-app.sh` requires the build number to be greater than every identity it can inspect across all local refs and reflogs; every merge parent is therefore covered. A rebase, squash, or cherry-pick that creates a new candidate SHA must advance the build number, including when the old candidate commit is no longer present in a fresh clone. A modern exact clean commit carrying its own live-origin `v*` release tag must be greater than every visible revision that is not its descendant and may not share its version/build pair with any other visible source. True later descendants with higher builds do not block rebuilding the older tag; divergent branches are not treated as descendants. A legacy-format source is accepted only at an exact live-origin `v<marketing-version>` tag with matching Swift marketing-version and plist version/build evidence. That historical path permits the same legacy pair on its ancestors or descendants, while still rejecting divergent same-pair history, unadvanced non-descendant identities, and reuse by another live-origin release tag. To read a built candidate's identity without copying values into documentation or commands, run:
