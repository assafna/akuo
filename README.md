...
Akuo observes an unfinished token until you type whitespace or Return, the only correction boundaries in version 1. Printable punctuation remains part of that token so URLs, email addresses, paths, domains, and identifiers reach the exclusion policy intact. While the token is unfinished, Akuo also keeps its transient physical key positions in memory so recoverable English capitals are not lost when Apple's Hebrew Shift layer emits different or no visible text. At a whitespace or Return boundary, Akuo maps the complete token through the other layout once. Structured text and unsupported or punctuation-dominated shapes remain excluded, while punctuation-shaped physical keys may remain eligible when the complete conversion is a target-layout word shape: letters only, or English letter segments joined by internal straight or curly apostrophes. A complete physical trace may also establish that a joined English word came from the intended letter and apostrophe keys, including Apple's silent and composite Shift outputs; malformed, edited, and text-only punctuation remains excluded. Akuo normally corrects only when the original is unknown and the mapped candidate is recognized by Akuo's seed vocabulary or the selected local macOS spelling dictionary. A complete physical trace with recovered Shift can also authorize a recognized English candidate when the visible Hebrew fragment is itself recognized, and a complete trace resolves the standalone English words `a` and `i`. Ordinary ambiguity without that physical evidence remains unchanged. macOS user-learned words participate in both directions. Recognition remains local, but exact coverage can vary by macOS version, installed language resources, and learned vocabulary. After a successful correction, Akuo selects the matching macOS input source so the next word starts in the intended language. Each physical key is interpreted with AppKit's current supported input source; if the exact source is unavailable or changes while a key is decoded, Akuo discards partial token state instead of trusting a potentially stale event text payload. If a requested source switch fails, the visible correction remains and the menu asks you to select a language manually. Every completed token is considered independently, so one sentence may alternate naturally between English and Hebrew.
