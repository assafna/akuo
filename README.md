...
A rebuild of the exact same clean Git commit with the same identity is the same candidate. For an Unreleased build, `Scripts/build-app.sh` requires the build number to be greater than every identity it can inspect across all local refs and reflogs; every merge parent is therefore covered. A rebase, squash, or cherry-pick that creates a new candidate SHA must advance the build number, including when the old candidate commit is no longer present in a fresh clone. A modern exact clean commit carrying its own live-origin `v*` release tag remains rebuildable when its build advanced beyond every ancestor and no other visible source revision used the same version/build pair; later descendants with higher builds do not block rebuilding the older tag. Legacy exact tags that stored their build in the plist remain rebuildable across their historical merge topology even when an ancestor carries the same pair, but another live-origin release tag may not reuse that pair. To read a built candidate's identity without copying values into documentation or commands, run:
