...
Immediately after an Akuo correction, Command-Z in the same application and focused field restores the original word, its boundary, and the previous input source. Akuo keeps only one transient undo record for up to five seconds. Any ordinary typing, Tab or Shift-Tab, mouse click, focus or application change, second correction, timeout, or Secure Input transition invalidates it. Once invalidated, Command-Z is passed to the current application normally.
